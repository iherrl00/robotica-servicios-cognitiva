...
class DialogueNode(Node):
    ...
    def handle_user_decision(self, request, response):
        ...
        try:
            ...
            # Para selección de waypoint manual
            elif 'qué punto' in prompt.lower() or 'a qué punto' in prompt.lower() or 'waypoints disponibles' in prompt.lower():
                response.decision = user_input
                # No necesita confirmación hablada
            ...
            elif 'qué punto' in prompt.lower() or 'cual punto' in prompt.lower() or 'waypoints disponibles' in prompt.lower():
                # Devolver la respuesta tal cual (puede ser número)
                response.decision = user_input
