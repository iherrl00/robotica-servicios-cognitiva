...
"""
Nodo de navegacion
Action Server que ejecuta navegacion con Nav2
"""
...
class NavigationNode(Node):
    def __init__(self):
        ...
        # Cliente para cambiar planificadores
        ...
        self.current_planner = 'GridBased'
        ...
        self.get_logger().info('Navigation Node iniciado (con cambio planificadores)')
    ...
    def execute_navigation(self, goal_handle):
        """Ejecuta navegacion con cambio inteligente de planificadores"""
        ...
        # Feedback loop con cambio inteligente de planificadores
        ...
        while not self.navigator.isTaskComplete():
            ...
            if nav_feedback:
                ...
                # CRITERIO 1: Distancia larga > 0.5m -> GridBased
                if remaining > 0.5 and not planner_changed:
                    if self.current_planner != 'GridBased':
                        self.get_logger().info('Distancia larga, usando GridBased')
                        self.change_planner('GridBased')
                        planner_changed = True
                
                # CRITERIO 2: Cerca < 0.3m -> SmacPlanner
                elif remaining < 0.3:
                    if self.current_planner != 'SmacPlanner':
                        self.get_logger().info('Aproximacion final, usando SmacPlanner')
                        self.change_planner('SmacPlanner')
                
                # CRITERIO 3: Atascado -> Alternar
                if self.detect_stuck(remaining):
                    self.get_logger().warn('Cambiando planificador por atascamiento')
                    if self.current_planner == 'GridBased':
                        self.change_planner('SmacPlanner')
                    else:
                        self.change_planner('GridBased')
